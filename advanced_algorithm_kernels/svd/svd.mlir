...
module {
  ...
  func.func @matrix_mul_trans_a(
   ...
   %res = linalg.matmul
           indexing_maps = [
             affine_map<(m, n, k) -> (k, m)>,   // A trasposta: legge (k,m)
             affine_map<(m, n, k) -> (k, n)>,   // B normale
  ...
  func.func @linalg_svd_pulp_open_fc(
      %src: tensor<?x?xf32>
  ) -> (tensor<?x?xf32>, tensor<?x?xf32>, tensor<?xf32>) 
  attributes { llvm.emit_c_interface } {

    %tmp = func.call @matrix_mul_trans_a(%src, %src) 
           : (tensor<?x?xf32>, tensor<?x?xf32>) -> tensor<?x?xf32>

    %diag_discarded, %mat_V, %vec_S = func.call @linalg_svd_jacobi(%tmp) 
           : (tensor<?x?xf32>) -> (tensor<?x?xf32>, tensor<?x?xf32>, tensor<?xf32>)

    %dst = func.call @linalg_svd_lsv(%src, %mat_V, %vec_S) 
           : (tensor<?x?xf32>, tensor<?x?xf32>, tensor<?xf32>) -> tensor<?x?xf32>

    return %dst, %mat_V, %vec_S : tensor<?x?xf32>, tensor<?x?xf32>, tensor<?xf32>
  }
