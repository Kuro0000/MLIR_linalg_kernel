...
extern void _mlir_ciface_matrix_mul_trans_a(
...
int main(int argc, char **argv) {
  ...
  StridedMemRefType_f32_2D B = make_memref_2d(M, P);
  StridedMemRefType_f32_2D C = make_memref_2d(N, P);
  StridedMemRefType_f32_2D Cref = make_memref_2d(N, P);

  for (int i = 0; i <= 6; i++) {
      random_fill(&A, 42 + i);
      random_fill(&B, 99 + i);

      matrix_mul_trans_A_pulp_open_fc(A.aligned, B.aligned, Cref.aligned, (int)M, (int)N, (int)P);
      _mlir_ciface_matrix_mul_trans_a(&C, &A, &B);
      ...
      for (long long n = 0; n < N; n++) {
        for (long long p = 0; p < P; p++) {
          printf("%7.2f ", Cref.aligned[n * P + p]);
        }
        printf("\n");
      }
      printf("\n");
