...
extern void _mlir_ciface_vector_add(StridedMemRefType_f32_1D *out,
...
//Golden Model 
static void ref_vector_add(const StridedMemRefType_f32_1D *A,
  ...
  StridedMemRefType_f32_1D Cref = make_memref_1d(N);
    for (long long i = 0; i <= 6; i++) {
          random_fill(&A, 13);
            ...
            // Esecuzione MLIR
            _mlir_ciface_vector_add(&C, &A, &B);
            
            // Esecuzione Golden Standard in C
            ref_vector_add(&A, &B, &Cref);
