...
extern void _mlir_ciface_linalg_gemv_trans(
    StridedMemRefType_f32_1D *out,
    StridedMemRefType_f32_2D *mat,
    StridedMemRefType_f32_1D *vec_x, // Dimensione M
    StridedMemRefType_f32_1D *vec_y, // Dimensione N
    float alpha,
    float beta
);
...
static StridedMemRefType_f32_1D make_memref_1d(long long size) {
...
int main(int argc, char **argv) {
    ...
    StridedMemRefType_f32_1D VecX  = make_memref_1d(M);
    StridedMemRefType_f32_1D VecY  = make_memref_1d(N);
    
    float *ref_dst = (float*)malloc(N * sizeof(float));
    ...
    for (int i = 0; i < num_tests; i++) {
        ...
        linalg_gemv_trans_pulp_open_fc(Mat.aligned, VecX.aligned, VecY.aligned, alpha, beta, ref_dst, (int)M, (int)N);

        StridedMemRefType_f32_1D mlir_dst = {0}; 
        _mlir_ciface_linalg_gemv_trans(&mlir_dst, &Mat, &VecX, &VecY, alpha, beta);
        ...
        if (err < 1e-3) {
            ...
            print_vector_head("  MLIR Output", mlir_dst.aligned, N, 8);
            print_vector_head("  C Reference", ref_dst, N, 8);
        }
