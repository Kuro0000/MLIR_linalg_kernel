...
extern void _mlir_ciface_vector_offset(StridedMemRefType_f32_1D *out,
...
// Golden Model 
static void ref_vector_offset(StridedMemRefType_f32_1D *C, StridedMemRefType_f32_1D *A, float val) {
    long long len = C->sizes[0];
    for (int i = 0; i < len; i++) {
        C->aligned[C->offset + i * C->strides[0]] = A->aligned[A->offset + i * A->strides[0]] + val;
    }
}
...
int main(int argc, char **argv) {
  ...
  // Eseguiamo 3 test con 3 numeri randomici diversi
